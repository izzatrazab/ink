...
A Drill's number-picking logic (the **Generator**) lives in `src/lib/questions/` as pure functions that return a **Question** (`{operands, operator, answer}`) and contain no `pdfkit`, no server-only code, and no `$lib` alias imports. The drill classes under `src/lib/server/` only _draw_ pre-generated Questions.
